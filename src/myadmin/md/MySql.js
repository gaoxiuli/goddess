...
var CONNECT_STR = "mongodb://localhost:27017/h5-1719";
...
//  moreValData   {$or:[{year:"1993"},{year:"1994"}]}
  ...
  updateData(db,collectionName,whereupdata,showupdata,callback){
			db.collection(collectionName).update(whereupdata,showupdata,(err,result) =>{
				if(err){
					console.log(err);
				}else{
					callback(result);
